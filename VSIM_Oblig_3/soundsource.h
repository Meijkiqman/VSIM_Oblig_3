...
public:
    ...
    **/
    ...
    void setPosition(Vector3D newPos);             ///< Sets source position from Vector3.
    Vector3D getPosition() {return mPosition;}     ///< Returns source position as Vector3.
    void setVelocity(Vector3D newVel);             ///< Sets source velocity from Vector3.
    Vector3D getVelocity() {return mVelocity;}     ///< Returns source velocity as Vector3.

private:
    std::string mName;          ///< The name of the sound source (Not used).
    ALuint mSource;             ///< The sound source.
    ALuint mBuffer;             ///< The data buffer.
    Vector3D mPosition;    ///< Vector containing source position.
    Vector3D mVelocity;    ///< Vector containing source velocity.
